...
const APPROVAL_MESSAGE = "Great news! We have reviewed your request and we are ready to proceed with your project.";
const NEED_INFO_MESSAGE = "We have reviewed your request and need some additional information before we can proceed. The admin will follow up shortly with the requirements needed.";
const DECLINE_MESSAGE = "Your request has been declined. We cannot proceed with this request at the moment. You can submit a new request if appropriate.";
...
router.patch("/users/:id/role", requireOwner, async (req, res): Promise<void> => { const id = parseInt(req.params.id as string); const { role } = req.body; if (!["owner", "admin", "freelancer", "client"].includes(role)) { res.status(400).json({ error: "Invalid role" }); return; } const [user] = await db.select().from(usersTable).where(eq(usersTable.id, id)); if (!user) { res.status(404).json({ error: "Not found" }); return; } if (user.role === "owner" && role !== "owner") { const ownerCount = await db.select({ count: count() }).from(usersTable).where(eq(usersTable.role, "owner")); if (Number(ownerCount[0].count) <= 1) { res.status(400).json({ error: "Cannot demote the last owner" }); return; } } const [updated] = await db.update(usersTable).set({ role }).where(eq(usersTable.id, id)).returning(); const [projectsCountRow] = await db.select({ count: count() }).from(projectsTable).where(eq(projectsTable.userId, id)); res.json(formatAdminUser(updated, Number(projectsCountRow.count))); });
...
router.get("/team", requireAdmin, async (req, res): Promise<void> => { const staff = await db.select().from(usersTable).where(sql`${usersTable.role} IN ('owner', 'admin', 'freelancer')`).orderBy(usersTable.createdAt); const assignments = await db.select({ freelancerId: projectAssignmentsTable.freelancerId, total: count() }).from(projectAssignmentsTable).groupBy(projectAssignmentsTable.freelancerId); const assignMap = new Map(assignments.map(a => [a.freelancerId, Number(a.total)])); res.json(staff.map(u => ({ id: u.id, name: u.name, email: u.email, role: u.role, isActive: u.isActive, avatarUrl: u.avatarUrl, assignedProjects: assignMap.get(u.id) ?? 0, createdAt: u.createdAt.toISOString(), lastLoginAt: u.lastLoginAt ? u.lastLoginAt.toISOString() : null }))); });
...
router.patch("/projects/:id", requireAdmin, async (req, res): Promise<void> => { const id = parseInt(req.params.id as string); const { status, priority, internalNotes, price } = req.body; const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, id)); if (!project) { res.status(404).json({ error: "Not found" }); return; } const updates: Record<string, unknown> = {}; if (status !== undefined) updates.status = status; if (priority !== undefined) updates.priority = priority; if (internalNotes !== undefined) updates.internalNotes = internalNotes; if (price !== undefined) updates.price = price; const [updated] = await db.update(projectsTable).set(updates).where(eq(projectsTable.id, id)).returning(); if (status && status !== project.status) { await db.insert(activityTable).values({ userId: project.userId, projectId: id, type: "status_change", description: `Project status changed to ${status}: ${project.title}` }); await db.insert(notificationsTable).values({ userId: project.userId, projectId: id, title: "Project Update", message: `Your project \"${project.title}\" status changed to ${status.replace("_", " ")}`, type: "status_change" }); } res.json(updated); });
...
router.post("/projects/:id/review", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (project.status !== "pending_review") { res.status(400).json({ error: `This request is already ${project.status.replace("_", " ")}` }); return; }

  const status = action === "approve" ? "approved" : action === "request_info" ? "needs_info" : "declined";
  const message = action === "approve" ? APPROVAL_MESSAGE : action === "request_info" ? NEED_INFO_MESSAGE : DECLINE_MESSAGE;
  const shouldOpenConversation = action !== "decline";

  await db.transaction(async (tx) => {
    await tx.update(projectsTable).set({ status, hasConversation: shouldOpenConversation }).where(eq(projectsTable.id, id));
    const senderId = req.userId;
    if (!senderId) throw Object.assign(new Error("Unauthorized"), { statusCode: 401 });

    if (action !== "decline") {
      await tx.insert(messagesTable).values({ projectId: id, senderId, content: message, isRead: false });
    }

    await tx.insert(activityTable).values({ userId: project.userId, projectId: id, type: "admin_response", description: `Request ${status}: ${project.title}` });
    await tx.insert(notificationsTable).values({ userId: project.userId, projectId: id, title: action === "approve" ? "Request Approved" : action === "request_info" ? "More Information Needed" : "Request Declined", message, type: action === "request_info" ? "new_message" : "status_change" });
  }).catch((error) => {
    const statusCode = typeof error === "object" && error !== null && "statusCode" in error ? Number((error as { statusCode?: number }).statusCode) : 500;
    if (statusCode) { throw Object.assign(new Error(error instanceof Error ? error.message : String(error)), { statusCode }); }
    throw error;
  });

  res.json({ ok: true, status, hasConversation: shouldOpenConversation });
});

router.post("/projects/:id/start-conversation", requireAdmin, async (req, res): Promise<void> => { const id = parseInt(req.params.id as string); const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, id)); if (!project) { res.status(404).json({ error: "Not found" }); return; } if (project.hasConversation) { res.json({ ok: true }); return; } await db.update(projectsTable).set({ hasConversation: true }).where(eq(projectsTable.id, id)); await db.insert(notificationsTable).values({ userId: project.userId, projectId: id, title: "Conversation Started", message: `A conversation has been started for your project \"${project.title}\"`, type: "message" }); res.json({ ok: true }); });
