...
function formatAdminUser(u: typeof usersTable.$inferSelect, totalRequests = 0) {
  return {
    id: u.id, name: u.name, email: u.email, userType: u.userType, location: u.country,
    companySize: u.companySize, acquisitionSource: u.acquisitionSource, registrationDate: u.createdAt.toISOString(),
    totalRequests, role: u.role, isActive: u.isActive, avatarUrl: u.avatarUrl, companyName: u.companyName,
    country: u.country, lastLoginAt: u.lastLoginAt ? u.lastLoginAt.toISOString() : null,
  };
}
...
router.get("/users/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  const recentActivity = await db.select({ id: activityTable.id, type: activityTable.type, description: activityTable.description, createdAt: activityTable.createdAt, projectId: activityTable.projectId, projectTitle: projectsTable.title })
    .from(activityTable).leftJoin(projectsTable, eq(activityTable.projectId, projectsTable.id)).where(eq(activityTable.userId, id)).orderBy(sql`${activityTable.createdAt} DESC`).limit(10);
...
router.patch("/users/:id/role", requireOwner, async (req, res): Promise<void> => {
  ...
  if (user.role === "owner" && role !== "owner") {
    ...
    if (Number(ownerCount[0].count) <= 1) { res.status(400).json({ error: "Cannot demote the last owner" }); return; }
  }
...
router.post("/users/:id/deactivate", requireOwner, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id as string); const { isActive } = req.body;
  await db.update(usersTable).set({ isActive }).where(eq(usersTable.id, id)); res.json({ ok: true });
});
...
router.get("/team", requireAdmin, async (req, res): Promise<void> => {
  ...
  const assignments = await db.select({ freelancerId: projectAssignmentsTable.freelancerId, total: count() }).from(projectAssignmentsTable).groupBy(projectAssignmentsTable.freelancerId);
...
router.get("/projects", requireAdmin, async (req, res): Promise<void> => {
  ...
  const projects = await db.select({
    id: projectsTable.id, projectCode: projectsTable.projectCode, title: projectsTable.title, serviceType: projectsTable.serviceType,
    description: projectsTable.description, requirements: projectsTable.requirements, status: projectsTable.status, priority: projectsTable.priority,
    price: projectsTable.price, internalNotes: projectsTable.internalNotes, hasConversation: projectsTable.hasConversation,
    clientName: usersTable.name, clientEmail: usersTable.email, createdAt: projectsTable.createdAt, updatedAt: projectsTable.updatedAt, userId: projectsTable.userId,
  }).from(projectsTable).innerJoin(usersTable, eq(projectsTable.userId, usersTable.id)).orderBy(sql`${projectsTable.createdAt} DESC`);

  const filtered = projects.filter(p => !status || p.status === status).filter(p => !priority || p.priority === priority);
  const projectIds = filtered.map(p => p.id);
  let assignmentMap = new Map<number, { id: number; name: string }[]>();
  if (projectIds.length > 0) {
    const assignments = await db.select({ projectId: projectAssignmentsTable.projectId, freelancerId: usersTable.id, freelancerName: usersTable.name })
      .from(projectAssignmentsTable).innerJoin(usersTable, eq(projectAssignmentsTable.freelancerId, usersTable.id)).where(inArray(projectAssignmentsTable.projectId, projectIds));
    for (const a of assignments) { if (!assignmentMap.has(a.projectId)) assignmentMap.set(a.projectId, []); assignmentMap.get(a.projectId)!.push({ id: a.freelancerId, name: a.freelancerName }); }
  }

  res.json(filtered.map(p => ({
    id: p.id, projectCode: p.projectCode, title: p.title, serviceType: p.serviceType, description: p.description, requirements: p.requirements,
    status: p.status, priority: p.priority, price: p.price !== null ? parseFloat(p.price) : null, internalNotes: p.internalNotes,
    hasConversation: p.hasConversation, clientName: p.clientName, clientEmail: p.clientEmail, assignedFreelancers: assignmentMap.get(p.id) ?? [],
...
router.patch("/projects/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (status !== undefined) updates.status = status; if (priority !== undefined) updates.priority = priority; if (internalNotes !== undefined) updates.internalNotes = internalNotes; if (price !== undefined) updates.price = price;
...
router.post("/projects/:id/start-conversation", requireAdmin, async (req, res): Promise<void> => {
  ...
  await db.update(projectsTable).set({ hasConversation: true }).where(eq(projectsTable.id, id));
  await db.insert(notificationsTable).values({ userId: project.userId, projectId: id, title: "Conversation Started", message: `A conversation has been started for your project "${project.title}"`, type: "message" });
  res.json({ ok: true });
});
