...
router.get("/projects", requireAdmin, async (req, res): Promise<void> => {
  ...
  if (projectIds.length > 0) {
    const assignments = await db.select({ projectId: projectAssignmentsTable.projectId, freelancerId: usersTable.id, freelancerName: usersTable.name }).from(projectAssignmentsTable).innerJoin(usersTable, eq(projectAssignmentsTable.freelancerId, usersTable.id)).where(inArray(projectAssignmentsTable.projectId, projectIds));
  ...
  const paymentRows = projectIds.length > 0 ? await db.select({ projectId: paymentsTable.projectId, status: paymentsTable.status }).from(paymentsTable).where(inArray(paymentsTable.projectId, projectIds)).orderBy(desc(paymentsTable.createdAt)) : [];
...
router.patch("/projects/:id", requireAdmin, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id as string); const { status, priority, internalNotes, price, paymentStatus } = req.body;
  if (paymentStatus !== undefined) { res.status(400).json({ error: "Payment status is managed by the payment flow and cannot be changed here" }); return; }
  if (status === "payment_pending" || status === "paid") { res.status(400).json({ error: "Payment status cannot be used as a project status" }); return; }
...
router.post("/projects/:id/review", requireAdmin, async (req, res): Promise<void> => {
  ...
  const message = action === "approve" ? "Great news! We have reviewed your request and we are ready to proceed with your project." : action === "request_info" ? NEED_INFO_MESSAGE : "We have reviewed your request and are unable to proceed with it at this time. You can submit a new request if you would like us to review a revised project request.";
  ...
  await db.transaction(async (tx) => {
    ...
    if (action !== "decline") await tx.insert(messagesTable).values({ projectId: id, senderId, content: message, isRead: false });
    ...
    await tx.insert(notificationsTable).values({ userId: project.userId, projectId: id, title: action === "approve" ? "Request Approved" : action === "request_info" ? "More Information Needed" : "Request Declined", message: action === "decline" ? "Your request has been declined. We cannot proceed with this request at the moment. You can submit a new request if appropriate." : message.split("\n")[0], type: action === "request_info" ? "new_message" : "status_change" });
  });
