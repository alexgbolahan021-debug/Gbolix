...
router.get("/projects/:projectId/offers", requireAuth, async (req, res): Promise<void> => { const projectId = parseOfferId(req.params.projectId); if (projectId === null) { res.status(400).json({ error: "Invalid project id" }); return; } const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, projectId)); if (!project) { res.status(404).json({ error: "Project not found" }); return; } if (req.userRole === "client" && project.userId !== req.userId) { res.status(403).json({ error: "Forbidden" }); return; } const offers = await db.select().from(offersTable).where(eq(offersTable.projectId, projectId)); res.json(offers.sort((a, b) => ((a.sentAt ?? a.createdAt).getTime() - (b.sentAt ?? b.createdAt).getTime()))); });

router.post("/offers/:offerId/send", requireAdmin, async (req, res): Promise<void> => {
  const offerId = parseOfferId(req.params.offerId); if (offerId === null) { res.status(400).json({ error: "Invalid offer id" }); return; }
  try {
    const result = await db.transaction(async (tx) => {
...
router.post("/offers/:offerId/withdraw", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      const [offer] = await tx.select().from(offersTable).where(eq(offersTable.id, offerId)); if (!offer) throw Object.assign(new Error("Offer not found"), { statusCode: 404 }); if (offer.status !== "sent") throw Object.assign(new Error("Only sent offers can be withdrawn"), { statusCode: 400 });
      const [project] = await tx.select().from(projectsTable).where(eq(projectsTable.id, offer.projectId)); if (!project) throw Object.assign(new Error("Project not found"), { statusCode: 404 }); if (!req.userId) throw Object.assign(new Error("Unauthorized"), { statusCode: 401 });
...
router.post("/offers/:offerId/accept", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      const [offer] = await tx.select().from(offersTable).where(eq(offersTable.id, offerId)); if (!offer) throw Object.assign(new Error("Offer not found"), { statusCode: 404 }); const [project] = await tx.select().from(projectsTable).where(eq(projectsTable.id, offer.projectId)); if (!project || project.userId !== req.userId) throw Object.assign(new Error("Forbidden"), { statusCode: 403 }); if (offer.status !== "sent") throw Object.assign(new Error("Only sent offers can be accepted"), { statusCode: 400 });
      const [updatedOffer] = await tx.update(offersTable).set({ status: "accepted", acceptedAt: new Date() }).where(and(eq(offersTable.id, offerId), eq(offersTable.status, "sent"))).returning(); if (!updatedOffer) throw Object.assign(new Error("Offer was already changed"), { statusCode: 409 });
      await tx.update(projectsTable).set({ status: "approved" }).where(eq(projectsTable.id, offer.projectId)); await tx.insert(activityTable).values({ userId: project.userId, projectId: project.id, type: "status_change", description: `Offer accepted for project: ${project.title}` }); return updatedOffer;
    });
    res.json({ offer: result, nextStep: "agreement" });
  } catch (error) { const statusCode = typeof error === "object" && error !== null && "statusCode" in error ? Number((error as { statusCode?: number }).statusCode) : 500; const message = error instanceof Error ? error.message : String(error); if (statusCode >= 400 && statusCode < 500) { res.status(statusCode).json({ error: message }); return; } res.status(500).json({ error: "Offer acceptance failed", detail: message }); }
});
...
router.post("/offers/:offerId/agreement", requireAdmin, async (req, res): Promise<void> => {
  const offerId = parseOfferId(req.params.offerId); if (offerId === null) { res.status(400).json({ error: "Invalid offer id" }); return; }
  const [offer] = await db.select().from(offersTable).where(eq(offersTable.id, offerId)); if (!offer) { res.status(404).json({ error: "Offer not found" }); return; } if (offer.status !== "accepted") { res.status(400).json({ error: "The offer must be accepted before an agreement is created" }); return; }
  const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, offer.projectId)); if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  const existing = await db.select().from(agreementsTable).where(eq(agreementsTable.projectId, project.id)); if (existing.length) { res.json(existing[0]); return; }
  const body = req.body ?? {}; const scope = String(body.scope ?? offer.scope ?? "").trim(); const deliverables = String(body.deliverables ?? offer.serviceName ?? "").trim(); const timeline = String(body.timeline ?? offer.deliveryEstimate ?? "").trim(); const revisions = String(body.revisions ?? "As agreed in the project scope").trim(); const price = String(body.price ?? offer.price ?? "").trim(); const terms = String(body.terms ?? offer.terms ?? "").trim();
  ...
  const [agreement] = await db.insert(agreementsTable).values({ projectId: project.id, scope, deliverables, timeline, revisions, price, terms }).returning(); await db.update(projectsTable).set({ status: "agreement_sent" }).where(eq(projectsTable.id, project.id)); const senderId = req.userId; if (!senderId) { res.status(401).json({ error: "Unauthorized" }); return; }
  await db.insert(messagesTable).values({ projectId: project.id, senderId, content: "Your project agreement is ready. Please review the agreement below and accept it to continue to payment.", isRead: false }); await db.insert(notificationsTable).values({ userId: project.userId, projectId: project.id, title: "Project Agreement Ready", message: `Your agreement for \"${project.title}\" is ready for review.`, type: "agreement" }); await db.insert(activityTable).values({ userId: project.userId, projectId: project.id, type: "admin_response", description: `Agreement sent for project: ${project.title}` }); res.status(201).json(agreement);
});

router.get("/projects/:projectId/agreement", requireAuth, async (req, res): Promise<void> => { const projectId = parseOfferId(req.params.projectId); if (projectId === null) { res.status(400).json({ error: "Invalid project id" }); return; } const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, projectId)); if (!project) { res.status(404).json({ error: "Project not found" }); return; } if (req.userRole === "client" && project.userId !== req.userId) { res.status(403).json({ error: "Forbidden" }); return; } const [agreement] = await db.select().from(agreementsTable).where(eq(agreementsTable.projectId, projectId)); if (!agreement) { res.status(404).json({ error: "Agreement not found" }); return; } res.json(agreement); });
router.post("/agreements/:agreementId/accept", requireAuth, async (req, res): Promise<void> => { const agreementId = parseOfferId(req.params.agreementId); if (agreementId === null) { res.status(400).json({ error: "Invalid agreement id" }); return; } const [agreement] = await db.select().from(agreementsTable).where(eq(agreementsTable.id, agreementId)); if (!agreement) { res.status(404).json({ error: "Agreement not found" }); return; } const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, agreement.projectId)); if (!project || project.userId !== req.userId) { res.status(403).json({ error: "Forbidden" }); return; } if (project.status !== "agreement_sent" || agreement.acceptedAt) { res.status(400).json({ error: "This agreement is not awaiting acceptance" }); return; } const [updatedAgreement] = await db.update(agreementsTable).set({ acceptedAt: new Date(), acceptedByUserId: req.userId }).where(eq(agreementsTable.id, agreementId)).returning(); await db.update(projectsTable).set({ status: "payment_pending" }).where(eq(projectsTable.id, project.id)); await db.insert(activityTable).values({ userId: project.userId, projectId: project.id, type: "status_change", description: `Agreement accepted for project: ${project.title}` }); res.json({ agreement: updatedAgreement, nextStep: "payment" }); });
