...
router.post("/", requireAuth, async (req, res): Promise<void> => {
  ...
  if (fileData && fileName) { const ext = path.extname(fileName); const unique = `${crypto.randomUUID()}${ext}`; const filePath = path.join(UPLOADS_DIR, unique); const buffer = Buffer.from(fileData, "base64"); fs.writeFileSync(filePath, buffer); fileUrl = `/api/files/download/${unique}`; savedFileName = fileName; savedMimeType = fileMimeType || "application/octet-stream"; }
