    ...
    <ClientLayout>
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold" data-testid="text-files-heading">Files</h1>
            <p className="text-muted-foreground text-sm mt-1">Upload and manage your project files.</p>
          </div>
          <div>
            <input
              ref={fileRef}
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {isLoading ? (
            <div className="space-y-px">
              {[1,2,3].map(i => <div key={i} className="h-14 bg-muted/30 animate-pulse" />)}
            </div>
          ) : !files?.length ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {files.map(f => (
                    <tr key={f.id} className="border-b border-border last:border-0 hover:bg-accent/20" data-testid={`row-file-${f.id}`}>
                      ...
                      <td className="px-5 py-4 text-muted-foreground text-xs">{formatBytes(f.size)}</td>
