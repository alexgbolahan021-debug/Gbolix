...
import { ArrowLeft, BookOpen, Bot, CheckCircle2, ChevronRight, Code2, Copy, ExternalLink, LayoutDashboard, MessageSquare, Plus, Rocket, Settings2, Sparkles, Trash2, Users, Zap } from "lucide-react";
...
export default function GbolixAIAgent() {
  ...
  async function deploy() {
    ...
    try {
      const token = await getToken();
      if (!token) throw new Error("Authentication token unavailable");
      const result = await agentRequest<{ embedCode: string; deployment: { tokenPrefix: string } }>(token, `/v1/agents/${selected.id}/deployments`, { method: "POST", body: JSON.stringify({}) });
  ...
  if (view === "list") return shell(<AgentList agents={agents} onNew={startNewAgent} onOpen={openAgent} />);
  return shell(<AgentDetail agent={selected} tab={tab} setTab={setTab} onBack={backToAgents} onNew={startNewAgent} form={form} setForm={setForm} knowledge={knowledge} knowledgeForm={knowledgeForm} setKnowledgeForm={setKnowledgeForm} usage={usage} chat={chat} message={message} setMessage={setMessage} deployment={deployment} onCreate={createAgent} onSave={saveAgent} onAddKnowledge={addKnowledge} onRemoveKnowledge={removeKnowledge} onSend={sendMessage} onResetChat={() => { setChat([]); setConversationId(undefined); }} onDeploy={deploy} busy={busy} goTo={goTo} />);
}

function AgentList({ agents, onNew, onOpen }: { agents: Agent[]; onNew: () => void; onOpen: (id: string) => void }) {
  const active = agents.filter((agent) => agent.status === "active").length;
  return <div className="space-y-7">
    <header className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><Sparkles size={14} /> Gbolix AI Agent</div><h1 className="text-3xl font-extrabold tracking-tight" style={{ fontFamily: "Sora, sans-serif" }}>Your AI agents</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Create and manage digital workers for your business. Choose an agent to open its workspace.</p></div><Button className="gap-2 self-start font-semibold" style={{ background: "linear-gradient(135deg,#00FF66,#00cc52)", color: "#0B0F14" }} onClick={onNew}><Plus size={16} /> Create new agent</Button></header>
...
function AgentDetail({ agent, tab, setTab, onBack, onNew, form, setForm, knowledge, knowledgeForm, setKnowledgeForm, usage, chat, message, setMessage, deployment, onCreate, onSave, onAddKnowledge, onRemoveKnowledge, onSend, onResetChat, onDeploy, busy, goTo }: any) {
  const creating = !agent;
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><button onClick={onBack} className="flex items-center gap-2 text-sm font-semibold text-muted-foreground transition hover:text-foreground"><ArrowLeft size={16} /> All agents</button><Button variant="outline" size="sm" className="gap-2" onClick={onNew}><Plus size={14} /> New agent</Button></div>
