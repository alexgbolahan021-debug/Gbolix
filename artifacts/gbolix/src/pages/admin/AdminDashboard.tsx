...
function money(values: Array<{ currency: string; amount: number }> | undefined) {
  ...
  return values.map(item => `${item.currency} ${item.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`).join(" · ");
}
      ...
      <div className="mx-auto max-w-7xl p-4 md:p-6">
        <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">Trend range</span>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          <div className="rounded-xl border border-border bg-card p-5 lg:col-span-2">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[{ label: "Paid", value: insights?.totalAmountPaid, icon: DollarSign, color: "text-primary" }, { label: "Pending", value: insights?.totalAmountPending, icon: Clock3, color: "text-yellow-400" }, { label: "Declined", value: insights?.totalAmountDeclined, icon: XCircle, color: "text-destructive" }, { label: "Total project value", value: insights?.projectValue, icon: Layers3, color: "text-blue-400" }].map(item => { const Icon = item.icon; return <div key={item.label} className="rounded-lg border border-border/80 bg-background/40 p-3"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Icon size={13} className={item.color} />{item.label}</div><p className="mt-2 text-sm font-bold">{money(item.value)}</p></div>; })}
            </div>
          </div>
