...
import { useAdminListProjects, useListMessages, useSendMessage, useGetMe } from "@workspace/api-client-react";
import { getAdminListProjectsQueryKey, getListMessagesQueryKey } from "@workspace/api-client-react";
...
export default function AdminMessages() {
  ...
  const { data: projects } = useAdminListProjects(
    {},
          ...
          <ScrollArea className="flex-1">
            {projectsWithConversation.length === 0 ? (
              <div className="p-4 text-center text-muted-foreground text-xs mt-4">
                No active conversations. Start one from the Projects page.
              </div>
            ) : (
              <div className="p-2 space-y-1">
                {projectsWithConversation.map(p => (
                  <button
                    ...
                    data-testid={`button-admin-conversation-${p.id}`}
                  ...
                  >
                    ...
                    <p className="text-[10px] text-muted-foreground">{p.serviceType}</p>
                  </button>
                ))}
              </div>
            )}
            ...
            <>
              <div className="px-5 py-4 border-b border-border">
                ...
                <p className="text-xs text-muted-foreground">
                  {selectedProject?.clientName} · {selectedProject?.serviceType}
                </p>
              </div>
