...
export default function AdminProjects() {
  ...
  const openPaymentEdit = (project: AdminProject) => { setEditingPaymentProject(project); setEditPaymentStatus(project.paymentStatus ?? "pending"); setEditPaymentReason(""); setEditPaymentAmount(project.price != null ? String(project.price) : ""); setEditPaymentCurrency("USD"); };
  ...
  const handlePaymentUpdate = async () => {
    ...
    if (!editingPaymentProject.paymentStatus && (!Number.isFinite(Number(editPaymentAmount)) || Number(editPaymentAmount) <= 0)) {
      toast({ title: "Amount required", description: "Enter the manual payment amount before creating a payment record.", variant: "destructive" });
      return;
    }
    ...
    try {
      const data = await customFetch<{ projectId: number; paymentId: number; paymentStatus: string; changed: boolean }>(apiUrl(`/admin/projects/${editingPaymentProject.id}/payment-status`), {
        ...
        body: JSON.stringify({ status: editPaymentStatus, reason: editPaymentReason.trim(), ...(!editingPaymentProject.paymentStatus ? { amount: Number(editPaymentAmount), currency: editPaymentCurrency } : {}) }),
      });
      refresh();
  ...
  const sendOffer = async () => {
    ...
    setOfferSaving(true);
    try {
      await customFetch<{ offer: any }>(apiUrl(`/projects/${offerProject.id}/offers`), {
        method: "POST",
        responseType: "json",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const openConversation = () => {
    ...
    window.location.href = `/admin/messages?project=${viewingProject.id}`;
  };
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative flex-1 min-w-48 max-w-72">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..." className="pl-8 h-9 text-sm" />
          </div>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-44 h-9 text-sm"><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {isLoading ? (
            <div className="p-4 space-y-2">{[1,2,3].map(i => <div key={i} className="h-14 bg-muted/30 animate-pulse rounded" />)}</div>
          ) : !filtered.length ? (
        ...
        <Dialog open={!!editingPaymentProject} onOpenChange={() => setEditingPaymentProject(null)}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{editingPaymentProject?.paymentStatus ? "Change payment status" : "Set payment status"}</DialogTitle></DialogHeader><p className="text-sm text-muted-foreground">This changes only the payment status for {editingPaymentProject?.title}. It does not change the project/request status and does not send a Paystack request.</p><div className="space-y-3">{editingPaymentProject && !editingPaymentProject.paymentStatus && <div className="grid gap-3 sm:grid-cols-[1fr_0.7fr]"><div><label className="text-sm font-medium mb-1.5 block">Amount</label><Input type="number" min="0.01" step="0.01" value={editPaymentAmount} onChange={event => setEditPaymentAmount(event.target.value)} placeholder="e.g. 39.00" /></div><div><label className="text-sm font-medium mb-1.5 block">Currency</label><Select value={editPaymentCurrency} onValueChange={setEditPaymentCurrency}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="USD">USD</SelectItem><SelectItem value="NGN">NGN</SelectItem></SelectContent></Select></div></div>}<div><label className="text-sm font-medium mb-1.5 block">Payment status</label><Select value={editPaymentStatus} onValueChange={setEditPaymentStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pending">Pending</SelectItem><SelectItem value="paid">Paid</SelectItem><SelectItem value="failed">Failed</SelectItem><SelectItem value="cancelled">Cancelled</SelectItem></SelectContent></Select></div><div><label className="text-sm font-medium mb-1.5 block">Reason</label><Textarea value={editPaymentReason} onChange={event => setEditPaymentReason(event.target.value)} rows={4} placeholder="Required audit reason, e.g. verified bank transfer" /></div></div><div className="flex gap-2 pt-2"><Button variant="outline" onClick={() => setEditingPaymentProject(null)} className="flex-1">Cancel</Button><Button onClick={() => void handlePaymentUpdate()} disabled={!editPaymentReason.trim() || paymentSaving} className="flex-1">{paymentSaving ? "Saving..." : "Save status"}</Button></div></DialogContent></Dialog>
...
function formatRequirements(req: any, description?: string | null) {
  ...
  return entries.map(([k, v]) => `${k.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase())}: ${Array.isArray(v) ? v.join(", ") : typeof v === "object" ? JSON.stringify(v) : String(v)}`).join("\n\n");
}
