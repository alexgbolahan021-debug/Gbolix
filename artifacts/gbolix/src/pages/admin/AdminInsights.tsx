...
import { CurrencyDisplayDisclosure, CurrencyToggle, convertMoney, formatMoney, type AdminCurrency } from "@/components/admin-currency";
...
export default function AdminInsights() {
  ...
  const toDisplayTrend = (points: Array<{ date: string; value: number; currency?: string }> | undefined) => (points ?? []).map(point => ({ ...point, value: convertMoney({ currency: point.currency ?? "USD", amount: point.value }, displayCurrency, displayRate) ?? 0 }));
  const revenue = toDisplayTrend(insights?.revenueOverTime);
  const projectValueTrend = toDisplayTrend(insights?.projectValueOverTime);
  const creditPurchaseTrend = toDisplayTrend(insights?.creditPurchaseValueOverTime);
  const projectValueDisplay = projectValue.map(item => ({ ...item, amount: convertMoney({ currency: item.currency, amount: item.amount }, displayCurrency, displayRate) ?? 0 }));
  const currencySymbol = displayCurrency === "USD" ? "$" : "₦";
  const paymentBreakdown = insights?.paymentValueBreakdown ?? [];
  ...
  return <ClientLayout>
    <div className="mx-auto max-w-7xl p-4 md:p-6">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2"><Card title="Requests over time"><div className="h-64">{insights?.requestsOverTime?.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={insights.requestsOverTime}><CartesianGrid strokeDasharray="3 3" stroke="#1e2a3a" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis allowDecimals={false} tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} /><Line type="monotone" dataKey="value" name="Requests" stroke="#FBBF24" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer> : <Empty text="No requests in this range." />}</div></Card><Card title="Project value over time"><div className="h-64">{projectValueTrend.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={projectValueTrend}><CartesianGrid strokeDasharray="3 3" stroke="#1e2a3a" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} formatter={(value: number) => [`${currencySymbol}${value.toLocaleString()}`, "Project value"]} /><Line type="monotone" dataKey="value" name="Project value" stroke="#60A5FA" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer> : <Empty text="No priced projects in this range." />}</div></Card></div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <Card title={`Project value by status (${displayCurrency})`}><div className="h-72">{projectValueDisplay.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={projectValueDisplay}><CartesianGrid strokeDasharray="3 3" stroke="#1e2a3a" /><XAxis dataKey="status" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} formatter={(value: number) => [`${currencySymbol}${value.toLocaleString()}`, "Value"]} /><Bar dataKey="amount" fill="#00FF66" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <Empty text="Project prices are not available yet." />}</div></Card>
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card title="Revenue over time"><div className="h-72">{revenue.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={revenue}><CartesianGrid strokeDasharray="3 3" stroke="#1e2a3a" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} formatter={(value: number) => [`${currencySymbol}${value.toLocaleString()}`, "Paid"]} /><Line type="monotone" dataKey="value" stroke="#00FF66" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer> : <Empty text="No successful payments in this range." />}</div></Card>
        <Card title="Wallet purchase value over time"><div className="h-72">{creditPurchaseTrend.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={creditPurchaseTrend}><CartesianGrid strokeDasharray="3 3" stroke="#1e2a3a" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} /><Line type="monotone" dataKey="value" stroke="#60A5FA" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer> : <Empty text="No successful credit purchases in this range." />}</div></Card>
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-2"><Card title="Payment value by status"><div className="space-y-3">{paymentBreakdown.map((item, index) => <div key={item.name} className="flex items-center justify-between rounded-lg border border-border p-3"><div><p className="text-sm font-medium">{item.name}</p><p className="text-xs text-muted-foreground">Canonical project payment records</p></div><p className={`text-sm font-bold ${index === 0 ? "text-primary" : index === 1 ? "text-yellow-400" : "text-destructive"}`}>{displayMoney(item.values)}</p></div>)}{!paymentBreakdown.length && <Empty text="No project payment records in this range." />}</div></Card><Card title="Wallet metrics"><div className="grid grid-cols-2 gap-3"><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Purchases</p><p className="mt-1 text-lg font-bold">{insights?.wallet.totalPurchases ?? 0}</p></div><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Credits purchased</p><p className="mt-1 text-lg font-bold">{(insights?.wallet.creditsPurchased ?? 0).toLocaleString()}</p></div><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Outstanding</p><p className="mt-1 text-lg font-bold">{(insights?.wallet.outstandingCredits ?? 0).toLocaleString()}</p></div><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Used</p><p className="mt-1 text-lg font-bold">{(insights?.wallet.creditsUsed ?? 0).toLocaleString()}</p></div></div></Card></div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <Card title="Customer locations and company size"><div className="grid gap-5 md:grid-cols-2"><div><p className="mb-2 text-xs font-medium text-muted-foreground">Location distribution</p>{insights?.locationBreakdown?.length ? <ResponsiveContainer width="100%" height={220}><BarChart data={insights.locationBreakdown} layout="vertical"><XAxis type="number" allowDecimals={false} tick={{ fontSize: 10 }} /><YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 10 }} /><Tooltip contentStyle={TooltipStyle()} /><Bar dataKey="value" fill="#34D399" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer> : <Empty text="No customer country data has been collected." />}</div><div><p className="mb-2 text-xs font-medium text-muted-foreground">Company / business size</p><Empty text="Unavailable: the current profile model does not collect company-size data." /></div></div></Card>
        <Card title="Wallet performance"><div className="space-y-4"><div className="flex items-center gap-3"><WalletCards size={18} className="text-primary" /><div><p className="text-xs text-muted-foreground">Purchase value</p><p className="text-sm font-bold">{displayMoney(insights?.wallet.purchaseValue)}</p></div></div><div className="flex items-center gap-3"><DollarSign size={18} className="text-blue-400" /><div><p className="text-xs text-muted-foreground">Average purchase</p><p className="text-sm font-bold">{displayMoney(insights?.wallet.averagePurchaseValue)}</p></div></div><div className="grid grid-cols-2 gap-3"><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Customers</p><p className="mt-1 text-lg font-bold">{insights?.wallet.purchasingCustomers ?? 0}</p></div><div className="rounded-lg border border-border p-3"><p className="text-[10px] uppercase text-muted-foreground">Credits used</p><p className="mt-1 text-lg font-bold">{(insights?.wallet.creditsUsed ?? 0).toLocaleString()}</p></div></div></div></Card>
      </div>
